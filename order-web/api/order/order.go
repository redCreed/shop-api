...
var (
	dtmServer = "192.168.18.100:36790"
	orderRpcServer = "192.168.18.100:50051"
	stockRpcServer = "192.168.18.100:50052"
)
...
func New(ctx *gin.Context) {
	...
	//order订单、stock库存服务的rpc在注册中心的地址
	//创建订单、扣减库存请求放入事务中 grpc形式请求，每个业务要有一个正向请求、一个回滚请求、以及请求参数，
	//当执行其中任何一个业务正向请求出错了会自动调用事务中所有业务回滚请求达到回滚效果。
	request := &proto.OrderRequest{}
	requestStock := &proto.SellInfo{}
	saga := dtmgrpc.NewSagaGrpc(dtmServer, gid).Add(
		orderRpcServer+"/Order/NewOrderFunc", orderRpcServer+"/Order/UpdateOrderStatus", request).
	   Add(stockRpcServer+"/Inventory/Sell", orderRpcServer+"/Inventory/Reback", requestStock)
	...
	//_, err := global.OrderSrvClient.CreateOrder(context.WithValue(context.Background(), "ginContext", ctx), &proto.OrderRequest{
